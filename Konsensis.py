...
@click.command()
@click.option('--models', default='1,2,3', help='Choose models to use: OpenAI is 1, Anthropic is 2, Cohere is 3 (use "1,2,3" for all).')
...
def main(models, recursive, threshold, initial_prompt, user_question):
    ...
    while True:
        ...
        api_key = openai_api_key if model_api == "OPENAI" else (anthropic_api_key if model_api == "ANTHROPIC" else cohere_api_key)
        model_name = 'gpt-4-0125-preview' if model_api == 'OPENAI' else ('claude-3-opus-20240229' if model_api == 'ANTHROPIC' else 'command-r-plus')
